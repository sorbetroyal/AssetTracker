    ...
    <div className="bg-zinc-900/40 border border-white/5 p-5 rounded-[2rem] flex flex-col sm:flex-row sm:items-center justify-between hover:bg-zinc-900/80 transition-all group relative overflow-hidden gap-6 sm:pr-24">
      ...
      <div className="flex flex-1 justify-between sm:justify-end items-center gap-8 md:gap-14">
        ...
        <div className="text-right min-w-[120px]">
          ...
          <div className="text-3xl font-black text-white italic tracking-tighter leading-none">₺{totalValue.toLocaleString('tr-TR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div>
        </div>
      </div>
...
function PortfolioListItem({ item, onDelete, isInvestment }: { item: PortfolioItem, onDelete: () => void, isInvestment?: boolean }) {
  ...
  return (
    <div className="bg-zinc-900/40 border border-white/5 p-5 rounded-[2rem] flex flex-col sm:flex-row sm:items-center justify-between hover:bg-zinc-900/80 transition-all group relative overflow-hidden gap-6 sm:pr-24">
      <div className="flex items-center gap-5">
        <div>
          <div className="text-3xl font-black text-white italic tracking-tighter ml-4 flex items-center gap-4 uppercase">
            {item.symbol.replace('.IS', '')}
            <span className="text-[10px] font-black text-blue-400/60 uppercase bg-blue-400/5 px-2.5 py-1 rounded-lg border border-blue-400/10 not-italic tracking-[0.2em]">
