...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // 2. TEFAS/BEFAS Fetcher
    if (fundSymbolsWithTypes.length > 0) {
      try {
        const fetchFund = async (item: { type: string, symbol: string }) => {
          const url = 'https://www.tefas.gov.tr/api/DB/GetAllFundAnalyzeData';
          const body = `dil=TR&fonkod=${item.symbol}`;
          ...
          return {
            symbol: item.symbol,
            type: item.type, // BEFAS gelirse BEFAS, TEFAS gelirse TEFAS döner
        ...
        const fundResults = await Promise.all(fundSymbolsWithTypes.map(fetchFund));
